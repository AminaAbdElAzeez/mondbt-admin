...
function CircularProgress({
  ...
  const safeValue = Number.isFinite(Number(percentage)) ? Number(percentage) : 0;
...
const AdminHome: React.FC = () => {
  ...
  const fetchStats = async (regionId?: number, schoolId?: number, studentId?: number) => {
    try {
      setStatsLoadingCrud(true);
      ...
      console.log('📊 fetchStats URL:', `/admin/home/statistic-of-filter?${queryString}`);

      const res = await axios.get(`/admin/home/statistic-of-filter?${queryString}`, {
        headers: {
          Authorization: `Bearer ${token || localStorage.getItem('token')}`,
          'Accept-Language': 'ar',
        },
  ...
  const handleSearch = async (regionId?: number, schoolId?: number, studentId?: number) => {
    try {
      setLoading(true);
      ...
      if (schoolId) params['filter[school_id]'] = schoolId;
      if (studentId) params['filter[student_id]'] = studentId; // 👈 أضفنا الطالب
      if (regionToUse !== null && regionToUse !== 0) params['filter[region]'] = regionToUse; // هنا region مش region_id
      ...
      if (res.data.status) {
        ...
        // ✅ هنا نفرق بين مدرسة و طالب
        if (studentId) {
          fetchStats(regionToUse, schoolId, studentId); // لو طالب
        } else {
          fetchStats(regionToUse, schoolId); // لو مدرسة
        }
      }
    } catch (err) {
