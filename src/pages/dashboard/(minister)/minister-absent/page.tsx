    ...
    const fetchAttendance = async () => {
      ...
      try {
        ...
        const res = await axios.get(
          `/minister/absent?filter[type]=${type}&region_id=${selectedRegionId}`,
          {
            headers: {
    ...
    <>
      {isLoading ? (
        ...
        <section dir="ltr" className="text-right px-2">
          <div className=" mb-5 md:mb-3 flex flex-col-reverse md:flex-row justify-end items-end lg:items-start gap-1 md:gap-2">
            <button
              onClick={async () => {
                try {
                  const res = await axios.get(
                    `/exports/absent?filter[type]=${filterTypeMapping[selected]}&region_id=${selectedRegionId}`,
                    {
                      headers: {
          ...
          <div className="flex flex-col xl:flex-row justify-between items-center xl:items-stretch  gap-6">
            <div
