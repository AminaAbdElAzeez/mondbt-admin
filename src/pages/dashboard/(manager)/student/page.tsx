...
const ManagerStudents: React.FC = () => {
  ...
  const [finesData, setFinesData] = useState<{ fines: ApiFine[]; total: number }>({
    fines: [],
    total: 0,
  });
  ...
  useEffect(() => {
    ...
    if (selectedAttend === 'ربيع الاول') {
      axios
        .get(`/manager/student/attendance-by-month/${id}`, {
          headers: {
            Authorization: `Bearer ${token || localStorage.getItem('token')}`,
            'Accept-Language': 'ar',
          },
        ...
        .then((res) => {
          console.log('month attendance:', res.data);
          setAttendance(res.data.data.attendance_percentage);
        });
    } else {
      axios
        .get(`/manager/student/attendance-by-year/${id}`, {
          headers: {
            Authorization: `Bearer ${token || localStorage.getItem('token')}`,
            'Accept-Language': 'ar',
          },
        ...
        .then((res) => {
          console.log('year attendance:', res.data);
          setAttendance(res.data.data.attendance_percentage);
        });
    }
  }, [id, selectedAttend]);
            ...
            <thead>
              <tr className="text-right" style={{ borderBottom: '1px solid #D9D9D9' }}>
                ...
                <th className="p-3 text-[#07A869] font-semibold text-xl flex justify-end items-center gap-1">
                  {finesData.total} <img src="/green-riyal.png" alt="icon" className="w-7 h-auto" />
                </th>
              </tr>
            </thead>
            <tbody>
              {finesData.fines.map((fine) => (
                <tr key={fine.id} className="hover:bg-[#f0eeee] transition-all duration-500">
                  <td className="p-3 text-[#15445A] font-semibold">{fine.type}</td>
                  <td className="p-3 text-[#15445A] font-medium text-left text-lg">
                    {fine.amount} <img src="/black-riyal.png" alt="icon" className="w-6 h-auto" />
                  </td>
                </tr>
              ))}
            </tbody>
