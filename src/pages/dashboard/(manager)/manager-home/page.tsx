...
const ManagerHome: React.FC = () => {
  ...
  const [grades, setGrades] = useState<
    Array<{ grade_id: number; grade_name: string; students: Array<{ id: number; name: string }> }>
  >([]);
  ...
  return (
    <section dir="ltr" className="text-right px-2">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-stretch gap-5 py-2 mb-5" dir='rtl'>

        <div
          className=" rounded-lg border flex flex-col gap-4 !w-full lg:!w-1/2"
          style={{ border: '1px solid #C2C1C1' }} dir='ltr'
        >
                  ...
                  >
                    ...
                    <div className="flex flex-col-reverse md:flex-row items-end md:items-center gap-3 w-[100%] sm:w-[80%]">
                      <div className="flex-1 flex flex-col justify-end items-end w-full">
                        <Link to={`/manager/student/${student.id}`} className="= sm:text-right font-semibold text-lg text-[#07A869] hover:text-[#15445A] transition-colors duration-500 mb-1.5 px-0">
                          {student.name}
                        </Link>
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto max-h-[400px]  px-4">
            <div className="flex items-center gap-4 justify-between px-4 mb-8">
              <CircularProgress
                percentage={selectedStudentStats ? selectedStudentStats.present : 0}
                size={100}
                strokeWidth={12}
                color="#07A869"
                bgColor="#E5E7EB"
            ...
            <div className="flex items-center gap-4 justify-between px-4 mb-8">
              <CircularProgress
                percentage={selectedStudentStats ? selectedStudentStats.absent : 0}
                size={100}
                strokeWidth={14}
                color="#07A869"
                bgColor="#E5E7EB"
            ...
            <div className="flex items-center gap-4 justify-between px-4 mb-8">
              <CircularProgress
                percentage={selectedStudentStats ? selectedStudentStats.rewards : 0}
                size={100}
                strokeWidth={14}
                color="#07A869"
                bgColor="#E5E7EB"
              />
              <span className="text-[#15445A] text-lg font-medium">المكافآت</span>
            </div>
          </div>
        </div>
      </div>
